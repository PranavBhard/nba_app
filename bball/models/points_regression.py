...
def get_feature_perspective(feature_name: str) -> str:
    """
    Extract the perspective component from a feature name.

    Feature format: stat_name|time_period|calc_weight|perspective[|side]

    Args:
        feature_name: Feature name string

    Returns:
        Perspective string: 'home', 'away', 'diff', or 'none'
    """
    if '|' not in feature_name:
        # Special features like pred_margin, Year, Month, Day
        return 'none'

    parts = feature_name.split('|')
    if len(parts) >= 4:
        # Standard format: perspective is 4th component (index 3)
        perspective = parts[3]
        # Handle |side suffix - perspective is still parts[3]
        if perspective in ['home', 'away', 'diff', 'none']:
            return perspective
        elif perspective == 'side' and len(parts) >= 5:
            # Edge case: if 'side' appears in position 3, check position 4
            # But normally 'side' is position 4, so this shouldn't happen
            return 'none'

    return 'none'


def split_features_by_perspective(feature_cols: List[str]) -> Tuple[List[str], List[str], List[int], List[int]]:
    """
    Split features into home-appropriate and away-appropriate subsets.

    Home model features: perspective in ['home', 'diff', 'none']
    Away model features: perspective in ['away', 'diff', 'none']

    Args:
        feature_cols: List of feature column names

    Returns:
        Tuple of (home_features, away_features, home_indices, away_indices)
    """
    home_features = []
    away_features = []
    home_indices = []
    ...
    for idx, feat in enumerate(feature_cols):
        perspective = get_feature_perspective(feat)

        if perspective in ['home', 'diff', 'none']:
            home_features.append(feat)
            home_indices.append(idx)

        if perspective in ['away', 'diff', 'none']:
            away_features.append(feat)
            away_indices.append(idx)

    return home_features, away_features, home_indices, away_indices
...
class PointsRegressionTrainer:
    ...
    def _build_top_ppg_cache(self, games: list):
        ...
        for (team, season), dates in teams_seasons.items():
            ...
            player_stats = list(self.db['nba_player_stats'].find(
                {
                    'team': team,
                    'season': season,
                    'stats.min': {'$gt': 0}
    ...
    def _get_top_ppg_for_team(self, team: str, season: str, before_date: str) -> float:
        ...
        try:
            ...
            result = list(self.db['nba_player_stats'].aggregate(pipeline))
    ...
    def _build_feature_vector(self, game: dict, before_date: str, selected_features: List[str] = None) -> Tuple[np.ndarray, List[str]]:
        ...
        Args:
            ...
            selected_features: List of feature names in pipe-delimited format (e.g., 'off_rtg|season|avg|home')
        ...
        if selected_features and len(selected_features) > 0:
            if not any('|' in f for f in selected_features):
                raise ValueError(f"Invalid feature format. Features must use pipe-delimited format (e.g., 'points|season|avg|home').")

        return self._build_feature_vector_internal(game, before_date, selected_features)
    ...
    def train(
        ...
        **model_kwargs
        ...
        # Split features by perspective for home/away models
        # Home model: features with perspective in ['home', 'diff', 'none']
        # Away model: features with perspective in ['away', 'diff', 'none']
        home_features, away_features, home_indices, away_indices = split_features_by_perspective(feature_cols)

        logger.info(f"Feature split by perspective:")
        logger.info(f"  Total features: {len(feature_cols)}")
        logger.info(f"  Home model features: {len(home_features)} (home + diff + none)")
        logger.info(f"  Away model features: {len(away_features)} (away + diff + none)")

        # Store feature names for each model
        self.home_feature_names = home_features
        self.away_feature_names = away_features
        self.feature_names = feature_cols  # Keep full list for compatibility
        ...
        if nan_count > 0:
            logger.warning(f"Found {nan_count} NaN values in feature matrix, filling with 0")
            X_full = np.nan_to_num(X_full, nan=0.0)

        X_home = X_full[:, home_indices]
        X_away = X_full[:, away_indices]
        ...
        X_home = X_home[sort_idx]
        X_away = X_away[sort_idx]
        y_home = y_home[sort_idx]
        y_away = y_away[sort_idx]

        # Compute temporal train mask BEFORE fitting scalers to prevent data leakage.
        # Scalers must be fit on training rows only so calibration/eval data
        # does not influence the mean/std.
        ...
        if use_time_calibration and calibration_years:
            ...
            earliest_cal_year = min([int(y) for y in (calibration_years if isinstance(calibration_years, list) else [calibration_years])])
            train_only_mask = (df_tmp['SeasonStartYear'] < earliest_cal_year).values
            print(f"[PointsRegressionTrainer.train] Scaler fit on {train_only_mask.sum()} training rows only (SeasonStartYear < {earliest_cal_year})")

        # Standardize features separately for each model
        # Fit on train rows only to prevent data leakage, transform all rows
        self.home_scaler = StandardScaler()
        self.away_scaler = StandardScaler()
        if train_only_mask is not None:
            self.home_scaler.fit(X_home[train_only_mask])
            self.away_scaler.fit(X_away[train_only_mask])
        else:
            self.home_scaler.fit(X_home)
            self.away_scaler.fit(X_away)
        X_home_scaled = self.home_scaler.transform(X_home)
        X_away_scaled = self.away_scaler.transform(X_away)

        # Keep legacy scaler for compatibility (fitted on full feature set)
        X_full_sorted = X_full[sort_idx]
        self.scaler = StandardScaler()
        if train_only_mask is not None:
            self.scaler.fit(X_full_sorted[train_only_mask])
        else:
            self.scaler.fit(X_full_sorted)
        X_scaled = self.scaler.transform(X_full_sorted)
        ...
        if target == 'margin':
            ...
            if model_type == 'Ridge':
                ...
                if use_time_based_split:
                    ...
                    # Train final model on training data ONLY (not calibration)
                    # This prevents data leakage when artifact is used as base model for stacking
                    logger.info(f"Training final margin-only model on training data only with best alpha={selected_alpha}")
                    print(f"[PointsRegressionTrainer.train] Final model trained on {len(train_idx)} training rows only")
                    self.model = Ridge(alpha=selected_alpha, random_state=42)
                    self.model.fit(X_train_split, y_train_m_split)
        ...
        if model_type == 'Ridge':
            ...
            if use_time_based_split:
                # Time-based calibration: train on train set, evaluate on calibration set, report evaluation set metrics
                # Use perspective-split features: home model gets home+diff+none, away model gets away+diff+none
                X_home_train_split = X_home_scaled[train_idx]
                X_away_train_split = X_away_scaled[train_idx]
                y_train_h_split = y_home[train_idx]
                y_train_a_split = y_away[train_idx]

                X_home_cal_split = X_home_scaled[cal_idx] if len(cal_idx) > 0 else None
                X_away_cal_split = X_away_scaled[cal_idx] if len(cal_idx) > 0 else None
                y_cal_h_split = y_home[cal_idx] if len(cal_idx) > 0 else None
                y_cal_a_split = y_away[cal_idx] if len(cal_idx) > 0 else None

                X_home_eval_split = X_home_scaled[eval_idx] if len(eval_idx) > 0 else None
                X_away_eval_split = X_away_scaled[eval_idx] if len(eval_idx) > 0 else None
                y_eval_h_split = y_home[eval_idx] if len(eval_idx) > 0 else None
                y_eval_a_split = y_away[eval_idx] if len(eval_idx) > 0 else None

                # Train models for each alpha value on training set
                for alpha in alphas:
                    logger.info(f"Training Ridge with alpha={alpha} using time-based calibration (perspective-split features)")

                    # Train home points model on home-perspective features
                    home_model = Ridge(alpha=alpha, random_state=42)
                    home_model.fit(X_home_train_split, y_train_h_split)

                    # Evaluate on calibration set
                    if X_home_cal_split is not None and len(X_home_cal_split) > 0:
                        y_pred_h_cal = home_model.predict(X_home_cal_split)
                    ...
                    # Train away points model on away-perspective features
                    away_model = Ridge(alpha=alpha, random_state=42)
                    away_model.fit(X_away_train_split, y_train_a_split)

                    # Evaluate on calibration set
                    if X_away_cal_split is not None and len(X_away_cal_split) > 0:
                        y_pred_a_cal = away_model.predict(X_away_cal_split)
                ...
                if X_home_eval_split is not None and len(X_home_eval_split) > 0:
                    print(f"[PointsRegressionTrainer.train] DIAGNOSTIC: Evaluating model (trained on train_idx only, {len(X_home_train_split)} samples) on evaluation set BEFORE retraining")
                    y_pred_h_eval_pre = best_result['home_model'].predict(X_home_eval_split)
                    y_pred_a_eval_pre = best_result['away_model'].predict(X_away_eval_split)
                ...
                # Train final model on training data ONLY (not calibration)
                # This prevents data leakage when artifact is used as base model for stacking
                logger.info(f"Training final model on training data only with best alpha={selected_alpha} (perspective-split features)")
                print(f"[PointsRegressionTrainer.train] Final model trained on {len(train_idx)} training rows only")
                X_home_train_final = X_home_scaled[train_idx]
                X_away_train_final = X_away_scaled[train_idx]
                y_train_h_final = y_home[train_idx]
                y_train_a_final = y_away[train_idx]

                self.model = {
                    'home': Ridge(alpha=selected_alpha, random_state=42),
                    'away': Ridge(alpha=selected_alpha, random_state=42)
                }
                self.model['home'].fit(X_home_train_final, y_train_h_final)
                self.model['away'].fit(X_away_train_final, y_train_a_final)

                # Re-evaluate the retrained model on evaluation set for final metrics
                if X_home_eval_split is not None and len(X_home_eval_split) > 0:
                    print(f"[PointsRegressionTrainer.train] Evaluating retrained model on evaluation set: {len(X_home_eval_split)} samples")
                    y_pred_h_eval = self.model['home'].predict(X_home_eval_split)
                    y_pred_a_eval = self.model['away'].predict(X_away_eval_split)
                    ...
                    print(f"[PointsRegressionTrainer.train] Final metrics from EVALUATION SET ({len(X_home_eval_split)} samples):")
                ...
                # Use TimeSeriesSplit CV (original behavior)
                # Train separate models for home and away points with perspective-split features
                for alpha in alphas:
                    logger.info(f"Training Ridge with alpha={alpha} (perspective-split features)")

                    # Train home points model on home-perspective features
                    home_model = Ridge(alpha=alpha, random_state=42)
                    home_cv_scores = {'mae': [], 'rmse': [], 'r2': [], 'mape': []}

                    for train_idx_cv, val_idx_cv in tscv.split(X_home_scaled):
                        X_train, X_val = X_home_scaled[train_idx_cv], X_home_scaled[val_idx_cv]
                    ...
                    for train_idx_cv, val_idx_cv in tscv.split(X_away_scaled):
                        X_train, X_val = X_away_scaled[train_idx_cv], X_away_scaled[val_idx_cv]
                ...
                # Retrain on full data with best alpha (perspective-split features)
                logger.info(f"Retraining on full data with best alpha={selected_alpha} (perspective-split features)")
                print(f"[PointsRegressionTrainer.train] WARNING: Time-based calibration DISABLED - retraining on FULL DATASET ({len(X_home_scaled)} samples)")
                self.model['home'] = Ridge(alpha=selected_alpha, random_state=42)
                self.model['away'] = Ridge(alpha=selected_alpha, random_state=42)
                self.model['home'].fit(X_home_scaled, y_home)
                self.model['away'].fit(X_away_scaled, y_away)

                # Final evaluation on full data
                print(f"[PointsRegressionTrainer.train] WARNING: Evaluating on FULL DATASET ({len(X_home_scaled)} samples, includes training data)")
                print(f"[PointsRegressionTrainer.train] This can give negative R² because model is evaluated on data it was trained on!")
                y_pred_home = self.model['home'].predict(X_home_scaled)
                y_pred_away = self.model['away'].predict(X_away_scaled)
            ...
            if use_time_based_split:
                ...
                self.model['home'].fit(X_train_split, y_train_h_split)
                self.model['away'].fit(X_train_split, y_train_a_split)
    ...
    def save_model(self, model_name: str = None):
        ...
        # Save perspective-specific scalers and feature names (for home_away models)
        if hasattr(self, 'home_scaler') and self.home_scaler is not None:
            home_scaler_path = os.path.join(self.artifacts_dir, f"{model_name}_home_scaler.pkl")
            with open(home_scaler_path, 'wb') as f:
                pickle.dump(self.home_scaler, f)

        if hasattr(self, 'away_scaler') and self.away_scaler is not None:
            away_scaler_path = os.path.join(self.artifacts_dir, f"{model_name}_away_scaler.pkl")
            with open(away_scaler_path, 'wb') as f:
                pickle.dump(self.away_scaler, f)

        if hasattr(self, 'home_feature_names') and self.home_feature_names:
            home_features_path = os.path.join(self.artifacts_dir, f"{model_name}_home_features.json")
            with open(home_features_path, 'w') as f:
                json.dump(self.home_feature_names, f)

        if hasattr(self, 'away_feature_names') and self.away_feature_names:
            away_features_path = os.path.join(self.artifacts_dir, f"{model_name}_away_features.json")
            with open(away_features_path, 'w') as f:
                json.dump(self.away_feature_names, f)
    ...
    def load_model(self, model_name: str):
        ...
        if self.home_feature_names and self.away_feature_names:
            logger.info(f"  Perspective-split features: home={len(self.home_feature_names)}, away={len(self.away_feature_names)}")
    ...
    def predict(self, game: dict, before_date: str, use_shap: bool = False, selected_features: List[str] = None) -> Dict:
        ...
        # Check if we have perspective-split features (new models)
        use_perspective_split = (
            self.target_type == 'home_away' and
            hasattr(self, 'home_feature_names') and self.home_feature_names and
            hasattr(self, 'away_feature_names') and self.away_feature_names and
            hasattr(self, 'home_scaler') and self.home_scaler is not None and
            hasattr(self, 'away_scaler') and self.away_scaler is not None
        )

        # Scale and predict
        if use_perspective_split:
            # Use perspective-split features for home/away models
            # Build home feature vector
            home_vector = np.array([feature_dict.get(name, 0.0) for name in self.home_feature_names])
            home_vector = np.nan_to_num(home_vector, nan=0.0, posinf=0.0, neginf=0.0)

            # Build away feature vector
            away_vector = np.array([feature_dict.get(name, 0.0) for name in self.away_feature_names])
            away_vector = np.nan_to_num(away_vector, nan=0.0, posinf=0.0, neginf=0.0)

            try:
                home_vector_scaled = self.home_scaler.transform(home_vector.reshape(1, -1))
                away_vector_scaled = self.away_scaler.transform(away_vector.reshape(1, -1))
            except Exception as e:
                logger.error(f"Error scaling perspective-split features: {e}")
                raise

            # Store for feature contributions later
            feature_vector_scaled = None  # Not used for perspective-split
        else:
            # Legacy: use full feature vector with single scaler
            if self.scaler is None:
            ...
            home_vector_scaled = None
            away_vector_scaled = None
        ...
        try:
            if self.target_type == 'margin':
                ...
                if use_perspective_split:
                    # Use perspective-appropriate scaled vectors
                    home_points_pred = self.model['home'].predict(home_vector_scaled)[0]
                    away_points_pred = self.model['away'].predict(away_vector_scaled)[0]
                else:
                    # Legacy: use same scaled vector for both
            ...
            if self.target_type == 'margin':
                ...
                logger.error(f"Model type: {type(self.model.get('home')) if isinstance(self.model, dict) else 'unknown'}, use_perspective_split: {use_perspective_split}")
            raise
        ...
        if self.target_type == 'margin':
            ...
            # Home/away models
            if use_perspective_split and hasattr(self.model['home'], 'coef_'):
                # Perspective-split linear model - use separate feature sets
                home_coef = self.model['home'].coef_
                away_coef = self.model['away'].coef_

                # Build contributions from home features
                for i, feature_name in enumerate(self.home_feature_names):
                    if feature_name not in contributions:
                        contributions[feature_name] = {}
                    contributions[feature_name]['home'] = float(home_coef[i] * home_vector_scaled[0, i])

                # Build contributions from away features
                for i, feature_name in enumerate(self.away_feature_names):
                    if feature_name not in contributions:
                        contributions[feature_name] = {}
                    contributions[feature_name]['away'] = float(away_coef[i] * away_vector_scaled[0, i])

                # Add 0.0 for missing contributions (e.g., |home features have no 'away' contribution)
                for feature_name in contributions:
                    if 'home' not in contributions[feature_name]:
                        contributions[feature_name]['home'] = 0.0
                    if 'away' not in contributions[feature_name]:
                        contributions[feature_name]['away'] = 0.0

            elif hasattr(self.model['home'], 'coef_'):
                # Legacy linear model - use same features for both
                home_coef = self.model['home'].coef_
                away_coef = self.model['away'].coef_
                ...
                try:
                    ...
                    if isinstance(self.model['home'], (RandomForestRegressor, xgb.XGBRegressor if XGBOOST_AVAILABLE else type(None))):
                        ...
                        if use_perspective_split:
                            explainer_home = shap.KernelExplainer(self.model['home'].predict, home_vector_scaled[:10])
                            explainer_away = shap.KernelExplainer(self.model['away'].predict, away_vector_scaled[:10])
                        else:
                    ...
                    if use_perspective_split:
                        shap_values_home = explainer_home.shap_values(home_vector_scaled)
                        shap_values_away = explainer_away.shap_values(away_vector_scaled)

                        # Build contributions from home features
                        for i, feature_name in enumerate(self.home_feature_names):
                            if feature_name not in contributions:
                                contributions[feature_name] = {}
                            contributions[feature_name]['home'] = float(shap_values_home[0][i]) if len(shap_values_home.shape) > 1 else float(shap_values_home[i])

                        # Build contributions from away features
                        for i, feature_name in enumerate(self.away_feature_names):
                            if feature_name not in contributions:
                                contributions[feature_name] = {}
                            contributions[feature_name]['away'] = float(shap_values_away[0][i]) if len(shap_values_away.shape) > 1 else float(shap_values_away[i])

                        # Fill missing contributions with 0.0
                        for feature_name in contributions:
                            if 'home' not in contributions[feature_name]:
                                contributions[feature_name]['home'] = 0.0
                            if 'away' not in contributions[feature_name]:
                    ...
                    if hasattr(self.model['home'], 'feature_importances_'):
                        ...
                        if use_perspective_split:
                            for i, feature_name in enumerate(self.home_feature_names):
                                if feature_name not in contributions:
                                    contributions[feature_name] = {}
                                contributions[feature_name]['home'] = float(home_importance[i] * home_vector_scaled[0, i])
                            for i, feature_name in enumerate(self.away_feature_names):
                                if feature_name not in contributions:
                                    contributions[feature_name] = {}
                                contributions[feature_name]['away'] = float(away_importance[i] * away_vector_scaled[0, i])
                            for feature_name in contributions:
                                if 'home' not in contributions[feature_name]:
                                    contributions[feature_name]['home'] = 0.0
                                if 'away' not in contributions[feature_name]:
    ...
    def get_feature_rankings(self) -> Dict:
        """
        Extract feature rankings from trained model(s).

        For home_away models with perspective-split features:
          - home_rankings: features sorted by abs coefficient/importance for home model
          - away_rankings: features sorted by abs coefficient/importance for away model

        For legacy or margin models:
          - rankings: features sorted by abs coefficient/importance
        ...
        if self.target_type == 'home_away' and isinstance(self.model, dict):
            # Check if we have perspective-split features
            use_perspective_split = (
                hasattr(self, 'home_feature_names') and self.home_feature_names and
                hasattr(self, 'away_feature_names') and self.away_feature_names
            )

            home_model = self.model.get('home')
            away_model = self.model.get('away')

            # Extract home model rankings
            if home_model is not None:
                if hasattr(home_model, 'coef_'):
                    # Linear model (Ridge, ElasticNet)
                    coef = home_model.coef_
                    feature_names = self.home_feature_names if use_perspective_split else self.feature_names
                    rankings = [(feature_names[i], float(coef[i])) for i in range(len(coef))]
                    rankings.sort(key=lambda x: abs(x[1]), reverse=True)
                    result['home_rankings'] = rankings
                    ...
                    feature_names = self.home_feature_names if use_perspective_split else self.feature_names
                    rankings = [(feature_names[i], float(importance[i])) for i in range(len(importance))]
                    rankings.sort(key=lambda x: x[1], reverse=True)
                    result['home_rankings'] = rankings

            # Extract away model rankings
            if away_model is not None:
                if hasattr(away_model, 'coef_'):
                    coef = away_model.coef_
                    feature_names = self.away_feature_names if use_perspective_split else self.feature_names
                    rankings = [(feature_names[i], float(coef[i])) for i in range(len(coef))]
                    rankings.sort(key=lambda x: abs(x[1]), reverse=True)
                    result['away_rankings'] = rankings
                elif hasattr(away_model, 'feature_importances_'):
        ...
        elif self.target_type == 'margin':
            # Single margin model
            if hasattr(self.model, 'coef_'):
                coef = self.model.coef_
                rankings = [(self.feature_names[i], float(coef[i])) for i in range(len(coef))]
                rankings.sort(key=lambda x: abs(x[1]), reverse=True)
                result['margin_rankings'] = rankings
            elif hasattr(self.model, 'feature_importances_'):
                importance = self.model.feature_importances_
                rankings = [(self.feature_names[i], float(importance[i])) for i in range(len(importance))]
                rankings.sort(key=lambda x: x[1], reverse=True)
                result['margin_rankings'] = rankings
