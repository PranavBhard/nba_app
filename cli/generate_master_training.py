...
Examples:
  ...
  python cli/generate_master_training.py --date-range 2024-01-01,2024-01-31
    ...
    if args.limit is not None and args.limit > 0:
        base_name = os.path.splitext(MASTER_TRAINING_PATH)[0]
        ...
        output_path = MASTER_TRAINING_PATH
    ...
    if args.date_range:
        if not os.path.exists(MASTER_TRAINING_PATH):
            print(f"Error: Master training CSV not found at {MASTER_TRAINING_PATH}. Generate it first without --date-range.")
            return
        ...
        try:
            ...
            if removed_count > 0:
                ...
                master_df_filtered.to_csv(MASTER_TRAINING_PATH, index=False)
            ...
            final_df = pd.read_csv(MASTER_TRAINING_PATH)
            ...
            print(f"Master CSV: {MASTER_TRAINING_PATH}")
            print()
