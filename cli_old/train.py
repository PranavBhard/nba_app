...
from nba_app.core.features.sets import (
    FEATURE_SETS,
    FEATURE_LAYERS,
...
def train_mode(args):
    ...
    # Initialize model with all possible features from registry SSoT
    all_features = get_all_possible_features(no_player=no_per)
    model = BballModel(
...
def predict_mode(args):
    """
    Predict mode: generate predictions for games on a given date.

    Uses PredictionService (core/prediction_service.py) as Single Source of Truth.
    Loads trained models from artifacts rather than retraining from CSV.
    """
    ...
    print("=" * 60)
    league = load_league_config(getattr(args, "league", None) or "nba")
    print(f"{league.display_name} Predictions for {pred_date}")
    if no_per:
    ...
    # Get and validate selected config
    configs = service.get_selected_configs()
    classifier_config = configs.get('classifier')
    points_config = configs.get('points')

    if classifier_config:
        print(f"\nUsing selected classifier config: {classifier_config.get('config_hash', 'unknown')}")
        print(f"  Model: {classifier_config.get('model_type', 'unknown')}")
        if classifier_config.get('best_c_value') is not None:
            print(f"  C-value: {classifier_config.get('best_c_value')}")

        # Show training stats if available
        if classifier_config.get('accuracy'):
            print(f"  Accuracy: {classifier_config['accuracy']:.2f}%")
        if classifier_config.get('log_loss'):
            print(f"  Log Loss: {classifier_config['log_loss']:.4f}")
    else:
        print("\nWarning: No selected classifier config found.")

    if points_config:
        print(f"\nUsing selected points config: {points_config.get('config_hash', 'unknown')}")
        print(f"  Model: {points_config.get('model_type', 'unknown')}")
    ...
    if not is_valid:
        print(f"\nError: {error_msg}")
        print("Please select a valid model config in the web UI or train a model first.")
        return

    # Generate predictions using PredictionService
    print(f"\nGenerating predictions...")
    results = service.predict_date(pred_date, include_points=True)

    if not results:
        print(f"No games found for {pred_date}")
        return

    # Convert PredictionResult objects to format expected by CLI helpers
    predictions = []
    errors = []
    ...
    # Build output structure for file writing
    model_type = classifier_config.get('model_type', 'Unknown') if classifier_config else 'Unknown'
    c_value = classifier_config.get('best_c_value') if classifier_config else None

    config_output = {
        'model_type': model_type,
        'c_value': c_value,
        'stats': {
            'accuracy': classifier_config.get('accuracy') if classifier_config else None,
            'accuracy_std': classifier_config.get('accuracy_std') if classifier_config else None,
            'log_loss': classifier_config.get('log_loss') if classifier_config else None,
            'brier': classifier_config.get('brier') if classifier_config else None
        },
        'predictions': predictions
    }
    all_prediction_outputs = [config_output]
    ...
    for pred in predictions:
        ...
        if pred.get('home_points_pred') is not None and pred.get('away_points_pred') is not None:
            line += f" | {pred['away_points_pred']:.0f}-{pred['home_points_pred']:.0f}"
        print(f"  {line}")

    if errors:
        print(f"\nErrors ({len(errors)}):")
        for err in errors:
            print(f"  {err['away']} @ {err['home']}: {err['error']}")
...
def generate_master_points_mode(args):
    ...
    from nba_app.cli_old.master_training_data_points import (
    ...
    if args.register_only:
        ...
        # Register in MongoDB
        from nba_app.cli_old.master_training_data_points import create_or_update_master_points_metadata
        from datetime import datetime
