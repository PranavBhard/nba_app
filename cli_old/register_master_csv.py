...
Usage:
    ...
    OR
    python -m nba_app.cli_old.register_master_csv (from project root)
"""
...
from nba_app.cli_old.master_training_data import register_existing_master_csv, MASTER_TRAINING_PATH, get_master_training_path
...
def main():
    ...
    # Usage:
    #   python cli/register_master_csv.py [--league nba] [optional_master_path]
    league_id = os.environ.get("LEAGUE_ID", "nba")
    ...
    # Get master path (can be overridden with optional positional arg)
    master_path = get_master_training_path(league)
    if len(argv) > 0:
        master_path = argv[0]
    ...
    try:
        metadata = register_existing_master_csv(db, master_path, league=league)
